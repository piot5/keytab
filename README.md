...
- **Likely Highlighting (theme settings section „Likely Highlighting")**: toggleable next-key highlighting — the most likely next key glows in an adjustable "Hervorhebung" color; when the typed word reaches the top suggestion (likelihood reached) a satisfying pulse effect (color flash + scale pulse + haptic) fires. Pure logic in `GamingLogic` (unit-tested).
